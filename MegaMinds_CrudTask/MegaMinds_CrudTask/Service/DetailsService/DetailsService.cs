...
namespace MegaMinds_CrudTask.Service.DetailsService
{
    public class DetailsService : IDetailsService
    {
		...
		public async Task<int> AddDetails(Details details)
        {
			...
			parameter.Add(new SqlParameter("@State", details.State));
			parameter.Add(new SqlParameter("@City", details.City));

			var result = await Task.Run(() => _dbContext.Database
		   .ExecuteSqlRawAsync(@"exec AddNewDetails @Name, @Email, @PhoneNumber, @Address , @State, @City", parameter.ToArray()));
        ...
        public async Task<int> UpdateDetails(Details details)
        {
			...
			var result = await Task.Run(() => _dbContext.Database
			.ExecuteSqlRawAsync(@"exec UpdateDetails @Id, @Name, @PhoneNumber, @Address, @State, @City", parameter.ToArray()));
			return result;
		}
    }
}
